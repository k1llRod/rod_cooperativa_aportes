...
class SetManagement(models.TransientModel):
    ...
    count_partner_cat_b = fields.Integer(string="Cantidad de socios categoria B", compute="compute_count_partner_cat_b")
    ...
    def compute_count_partner(self):
        for record in self:
            record.count_partner = self.env['partner.payroll'].search_count(
                [('partner_status_especific', '!=', 'passive_reserve_b'), ('state', '=', 'process')])

    @api.depends('set_management_id')
    def compute_count_partner_cat_b(self):
        for record in self:
            record.count_partner_cat_b = self.env['partner.payroll'].search_count(
                [('partner_status_especific', '=', 'passive_reserve_b'), ('state', '=', 'process')])